...
// 1. Veri Tipi Etiketi (Tensörün o an ne tuttuðunu bilmek için)
typedef enum {
    TIP_FLOAT32,
    TIP_FLOAT16,
    TIP_INT8
...
// 2. UNION MÝMARÝSÝ (Ayný bellek alanýný paylaþan veri tipleri)
typedef union {
    float f32;       
    uint16_t f16;    
    int8_t i8;       
...
typedef struct {
    ...
    int sutun;
    VeriTipi tip;
    TensorVerisi *veri; // Dinamik dizi pointer'ý
} Tensor;

// --- TENSÖR OLUÞTURMA (RAM'DE YER AYIRMA) ---
...
// --- BELLEK TEMÝZLEME (Memory Leak Önleme) ---
...
// --- QUANTIZATION (SIKIÞTIRMA) ALGORÝTMASI: Float32 -> Int8 ---
void tensor_kuantize_et(Tensor* kaynak_float, Tensor* hedef_int8) {
    ...
    float olcek;
    int i; // C89 standardý için döngü deðiþkeni en üste alýndý
...
int main() {
    ...
    // 2. Sýkýþtýrýlacak Int8 Tensörü Oluþtur
